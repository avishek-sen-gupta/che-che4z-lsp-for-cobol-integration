...
class IdmsCopybookService {
   ...
   */
  public ResultWithErrors<List<Node>> processCopybook(
      CopybookModel copybookModel, int parentLevel, Locality locality) {
    ...
    if (copybookModel.getContent() == null) {
      ...
      errors.add(
          ErrorHelper.missingCopybooks(messageService, locality, copybookName.getQualifiedName()));
    ...
    if (processedCopybooks.contains(copybookName)) {
      List<SyntaxError> errors = new LinkedList<>();
      errors.add(
          ErrorHelper.circularDependency(
              messageService, locality, copybookName.getQualifiedName()));
      return new ResultWithErrors<>(ImmutableList.of(), errors);
    }
    ...
    List<SyntaxError> errors = new LinkedList<>();
    processNodes(copybookModel, parentLevel).unwrap(errors::addAll).forEach(node::addChild);
