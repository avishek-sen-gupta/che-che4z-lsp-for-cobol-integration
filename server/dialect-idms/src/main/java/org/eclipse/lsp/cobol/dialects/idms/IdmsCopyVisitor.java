...
class IdmsCopyVisitor extends IdmsCopyParserBaseVisitor<List<IdmsCopybookDescriptor>> {
  ...
  @Getter private final List<Pair<Range, Integer>> variableLevels = new LinkedList<>();
  ...
  public List<IdmsCopybookDescriptor> visitCopyIdmsStatement(
    ...
    return ImmutableList.of(IdmsCopybookDescriptor.from(ctx, extendedDocument.getUri()));
  }
  ...
  @Override
  public List<IdmsCopybookDescriptor> visitDataDescriptionEntryFormat1(
      IdmsCopyParser.DataDescriptionEntryFormat1Context ctx) {
    variableLevels.add(
        Pair.of(
            getLevelLocality(ctx.levelNumber().LEVEL_NUMBER()).getRange(),
            VisitorHelper.getLevel(ctx.levelNumber().LEVEL_NUMBER())));
    return visitChildren(ctx);
  }

  @Override
  public List<IdmsCopybookDescriptor> visitDataDescriptionEntryFormat2(
  ...
  @Override
  public List<IdmsCopybookDescriptor> visitDataDescriptionEntryFormat3(
    ...
    ofNullable(ctx.dataValueClause())
        ...
        .ifPresent(
            t ->
                variableLevels.add(
                    Pair.of(getLevelLocality(ctx.LEVEL_NUMBER_88()).getRange(), LEVEL_88)));
    return visitChildren(ctx);
  }
  ...
  private Locality getLevelLocality(TerminalNode terminalNode) {
    return Locality.builder()
        .range(
            new Range(
                new Position(
                    terminalNode.getSymbol().getLine() - 1,
                    terminalNode.getSymbol().getCharPositionInLine()),
                new Position(
                    terminalNode.getSymbol().getLine() - 1,
                    terminalNode.getSymbol().getCharPositionInLine()
                        + terminalNode.getText().length())))
        .build();
  }
}
