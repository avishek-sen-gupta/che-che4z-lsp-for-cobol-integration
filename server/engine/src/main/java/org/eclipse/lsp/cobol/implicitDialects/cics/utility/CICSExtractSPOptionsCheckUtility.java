...
public class CICSExtractSPOptionsCheckUtility extends CICSOptionsCheckBaseUtility {
  ...
  private void checkSubResidOptions(CICSParser.Cics_extract_statisticsContext ctx) {
    ...
    List<TerminalNode> subrestype =
        subrestypes.stream()
            .map(CICSParser.Cics_subrestypeContext::SUBRESTYPE)
            .collect(Collectors.toList());
    List<TerminalNode> capturespec =
        subrestypes.stream()
            .map(CICSParser.Cics_subrestypeContext::CAPTURESPEC)
            .collect(Collectors.toList());
    List<TerminalNode> policyrule =
        subrestypes.stream()
            .map(CICSParser.Cics_subrestypeContext::POLICYRULE)
            .collect(Collectors.toList());

    checkHasMutuallyExclusiveOptions(
        "SUBRESTYPE or CAPTURESPEC or POLICYRULE", subrestype, capturespec, policyrule);
