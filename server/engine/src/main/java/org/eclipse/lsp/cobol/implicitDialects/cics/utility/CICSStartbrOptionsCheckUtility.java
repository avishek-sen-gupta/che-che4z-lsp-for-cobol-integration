...
public class CICSStartbrOptionsCheckUtility extends CICSOptionsCheckBaseUtility {
  ...
  public CICSStartbrOptionsCheckUtility(
  ...
  private void checkStartbr(CICSParser.Cics_startbr_optionsContext ctx) {
    ...
    if (!ctx.SYSID().isEmpty()) {
      checkHasExactlyOneOption(
          "KEYLENGTH, RBA, RRN, or XRBA", ctx, ctx.KEYLENGTH(), ctx.RBA(), ctx.RRN(), ctx.XRBA());
    }
    checkHasMutuallyExclusiveOptions(
        "GTEQ or DEBKEY or RBA or DEBREC", ctx.GTEQ(), ctx.DEBKEY(), ctx.RBA(), ctx.DEBREC());
  }
}
