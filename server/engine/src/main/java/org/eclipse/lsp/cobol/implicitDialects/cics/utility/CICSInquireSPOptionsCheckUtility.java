...
public class CICSInquireSPOptionsCheckUtility extends CICSOptionsCheckBaseUtility {
  ...
  private static final Map<Integer, Integer> COMMON_INQUIRE_BROWSE_RULES =
  ...
  public <E extends ParserRuleContext> void checkOptions(E ctx) {
    switch (ctx.getRuleIndex()) {
      ...
      case CICSParser.RULE_cics_inquire_statistics:
        CICSParser.Cics_inquire_statisticsContext statsContext =
            (CICSParser.Cics_inquire_statisticsContext) ctx;

        checkHasMutuallyExclusiveOptions(
            "ENDOFDAY or ENDOFDAYHRS", statsContext.ENDOFDAY(), statsContext.ENDOFDAYHRS());
        if (!statsContext.ENDOFDAYHRS().isEmpty()) {
          checkHasMandatoryOptions(
              statsContext.ENDOFDAYMINS(), statsContext, "ENDOFDAYMINS with ENDOFDAYHRS");
          checkHasMandatoryOptions(
              statsContext.ENDOFDAYSECS(), statsContext, "ENDOFDAYSECS with ENDOFDAYHRS");
        } else {
        ...
        checkHasMutuallyExclusiveOptions(
            "INTERVAL or INTERVALHRS", statsContext.INTERVAL(), statsContext.INTERVALHRS());
        if (!statsContext.INTERVALHRS().isEmpty()) {
          checkHasMandatoryOptions(
              statsContext.INTERVALMINS(), statsContext, "INTERVALMINS with INTERVALHRS");
          checkHasMandatoryOptions(
              statsContext.INTERVALSECS(), statsContext, "INTERVALSECS with INTERVALHRS");
        } else {
        ...
        checkHasMutuallyExclusiveOptions(
            "NEXTTIME or NEXTTIMEHRS", statsContext.NEXTTIME(), statsContext.NEXTTIMEHRS());
        if (!statsContext.NEXTTIMEHRS().isEmpty()) {
          checkHasMandatoryOptions(
              statsContext.NEXTTIMEMINS(), statsContext, "NEXTTIMEMINS with NEXTTIMEHRS");
          checkHasMandatoryOptions(
              statsContext.NEXTTIMESECS(), statsContext, "NEXTTIMESECS with NEXTTIMEHRS");
        } else {
      ...
      case CICSParser.RULE_cics_inquire_storage:
        CICSParser.Cics_inquire_storageContext storageContext =
            (CICSParser.Cics_inquire_storageContext) ctx;
        checkHasExactlyOneOption(
      ...
      case CICSParser.RULE_cics_inquire_storage64:
        CICSParser.Cics_inquire_storage64Context s64Context =
            (CICSParser.Cics_inquire_storage64Context) ctx;
        checkHasExactlyOneOption(
      ...
      case CICSParser.RULE_cics_inquire_transaction:
        ...
        checkHasMutuallyExclusiveOptions(
            "TCLASS and TRANCLASS", transactionContext.TCLASS(), transactionContext.TRANCLASS());
        break;
      ...
      case CICSParser.RULE_cics_inquire_vtam:
        ...
        checkHasMutuallyExclusiveOptions(
            "PSDINTHRS with PSDINTERVAL", vtamContext.PSDINTERVAL(), vtamContext.PSDINTHRS());
        checkPrerequisiteIsMet(
