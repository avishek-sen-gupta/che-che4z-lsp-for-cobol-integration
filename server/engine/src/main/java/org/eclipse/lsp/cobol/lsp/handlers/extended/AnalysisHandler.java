...
public class AnalysisHandler {
   ...
   */
  public ExtendedApiResult analysis(AnalysisResultEvent analysisResultEvent)
    ...
    Optional<Node> selectedNode =
        RangeUtils.findNodeByPosition(rootNode, analysisResultEvent.getUri(), position);
    ...
    return cfastBuilder.build(
        (ProgramNode)
            selectedNode
                .get()
                .getNearestParentByType(PROGRAM)
                .orElse(selectProgramNode(rootNode.findPrograms(), line, character)));
  }
