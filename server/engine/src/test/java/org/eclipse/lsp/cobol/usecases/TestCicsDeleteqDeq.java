 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-deleteq-td">DELETEQ Command</a> <a
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-deq">DEQ Command</a>
 *
...
public class TestCicsDeleteqDeq {
  ...
  private static final String DELETEQ_TD_SYSID_VALID =
      "DELETEQ TD QUEUE({$varFour}) SYSID({$varFive})";
  ...
  private static final String DELETEQ_QUEUE_SYSID_VALID =
      "DELETEQ QUEUE({$varFour}) SYSID({$varFive})";

  private static final String DELETEQ_QNAME_SYSID_VALID =
      "DELETEQ QNAME({$varFour}) SYSID({$varFive})";
  private static final String DELETEQ_QNAME_SYSID_INVALID =
      "DELETEQ QNAME({$varFour}) {QNAME|error1}({$varFour}) SYSID({$varFive})";
  private static final String DELETEQ_QUEUE_QNAME_INVALID =
      "DELETEQ {QUEUE|error1}({$varFour}) {QNAME|error2}({$varFour}) SYSID({$varFive})";

  private static final String DELETEQ_TS_QUEUE_SYSID_VALID =
      "DELETEQ TS QUEUE({$varFour}) SYSID({$varFive})";

  private static final String DELETEQ_TS_QNAME_SYSID_VALID =
      "DELETEQ TS QNAME({$varFour}) SYSID({$varFive})";

  private static final String DEQ_RESOURCE_VALID = "DEQ RESOURCE({$varFour})";

  private static final String DEQ_RESOURCE_LENGTH_VALID =
      "DEQ RESOURCE({$varFour}) LENGTH({$varTwo})";
  ...
  private static final String DEQ_RESOURCE_MAXLIFETIME_VALID =
      "DEQ RESOURCE({$varFour}) MAXLIFETIME({$varTwo})";

  private static final String DEQ_RESOURCE_LENGTH_UOW_VALID =
      "DEQ RESOURCE({$varFour}) LENGTH({$varTwo}) UOW";

  private static final String DEQ_RESOURCE_LENGTH_TASK_VALID =
      "DEQ RESOURCE({$varFour}) LENGTH({$varTwo}) TASK";

  private static final String DEQ_RESOURCE_LENGTH_MAXLIFETIME_VALID =
      "DEQ RESOURCE({$varFour}) LENGTH({$varTwo}) MAXLIFETIME({$varTwo})";
  private static final String DEQ_RESOURCE_LENGTH_UOW_TASK_INVALID =
      "DEQ RESOURCE({$varFour}) LENGTH({$varTwo}) {UOW|error1} {TASK|error2}";
  ...
  void testDeqResourceLengthUowTaskInvalid() {
    Map<String, Diagnostic> expectedDiagnostic =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: UOW or MAXLIFETIME or"
                    + " TASK",
