...
public class TestCICSInquire {
  private static Stream<String> getValidOptions() {
    return Stream.of(
        "ACTIVITYID({$varOne}) COMPSTATUS({$varOne}) MODE({$varOne}) SUSPSTATUS({$varOne})"
            + " ABCODE({$varOne}) ABPROGRAM({$varOne}) ACTIVITY({$varOne}) EVENT({$varOne})"
            + " PROCESS({$varOne}) PROCESSTYPE({$varOne}) PROGRAM({$varOne}) TRANSID({$varOne})"
            + " USERID({$varOne})",
        "CONTAINER({$varOne}) PROCESS({$varOne}) PROCESSTYPE({$varOne}) DATALENGTH({$varOne})"
            + " SET({$varOne})",
        "EVENT({$varOne}) ACTIVITYID({$varOne}) EVENTTYPE({$varOne}) FIRESTATUS({$varOne})"
            + " PREDICATE({$varOne}) COMPOSITE({$varOne}) TIMER({$varOne})",
  ...
  void testContainerInvalid() {
    Map<String, Diagnostic> expectedDiagnostics =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: ACTIVITYID and"
                    + " PROCESS",
