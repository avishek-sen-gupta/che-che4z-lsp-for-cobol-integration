 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-query">QUERY Command</a>
 *
 * <p>This class tests all variations of the QUERY command found in the link above.
 */
public class TestCicsQuery {
  private static final String QUERY_CHANNEL_VALID_ONE =
      "QUERY CHANNEL({$varFour}) CONTAINERCNT({$varSix})";
  private static final String QUERY_CHANNEL_VALID_TWO =
      "QUERY CONTAINERCNT({$varFour}) CHANNEL({$varSix}) ";
  ...
  private static final String QUERY_COUNTER_VALID_ALL_ONE =
      "QUERY COUNTER({$varOne}) POOL({$varFive}) VALUE({$varSix}) MINIMUM({$varTwo})"
          + " MAXIMUM({$varTwo}) NOSUSPEND";
  private static final String QUERY_COUNTER_VALID_ALL_TWO =
      "QUERY MINIMUM({$varTwo}) COUNTER({$varOne}) POOL({$varFive}) NOSUSPEND VALUE({$varSix}) "
          + " MAXIMUM({$varTwo}) ";
  private static final String QUERY_COUNTER_VALID_ALL_THREE =
      "QUERY VALUE({$varSix}) MINIMUM({$varTwo}) NOSUSPEND  MAXIMUM({$varTwo}) POOL({$varFive})"
          + " COUNTER({$varOne})";

  private static final String QUERY_DCOUNTER_VALID_ALL_ONE =
      "QUERY DCOUNTER({$varOne}) MINIMUM({$varTwo}) MAXIMUM({$varTwo}) POOL({$varFive})"
          + " VALUE({$varSix}) NOSUSPEND";
  private static final String QUERY_DCOUNTER_VALID_ALL_TWO =
      "QUERY NOSUSPEND POOL({$varFive}) MINIMUM({$varTwo}) DCOUNTER({$varOne}) MAXIMUM({$varTwo})"
          + " VALUE({$varSix})";

  private static final String QUERY_COUNTER_VALID_SOME_ONE = "QUERY COUNTER({$varOne}) NOSUSPEND";
  ...
  private static final String QUERY_COUNTER_INVALID_ONE =
      "QUERY {COUNTER|error1}(1) {DCOUNTER|error1}(1) NOSUSPEND";
  private static final String QUERY_COUNTER_INVALID_TWO =
      "QUERY VALUE(6) COUNTER({$varFour}) {VALUE|error1}(6)";
  ...
  private static final String QUERY_SECURITY_VALID_ALL_ONE =
      "QUERY SECURITY RESTYPE({$varTwo}) RESID({$varThree}) LOGMESSAGE({$varFour})"
          + " USERID({$varFour}) READ({$varSix}) UPDATE({$varFour}) CONTROL({$varTwo})"
          + " ALTER({$varFive})";
  private static final String QUERY_SECURITY_VALID_ALL_TWO =
      "QUERY RESID({$varThree}) CONTROL({$varTwo}) USERID({$varFour}) RESIDLENGTH({$varSix})"
          + " ALTER({$varFive}) RESCLASS({$varFour}) UPDATE({$varFour}) SECURITY"
          + " LOGMESSAGE({$varFour}) READ({$varSix})";
  private static final String QUERY_SECURITY_VALID_ALL_THREE =
      "QUERY ALTER({$varFive}) LOGMESSAGE({$varFour}) READ({$varSix}) SECURITY RESID({$varThree})"
          + " UPDATE({$varFour})RESTYPE({$varTwo}) USERID({$varFour}) CONTROL({$varTwo})";
  private static final String QUERY_SECURITY_VALID_ALL_FOUR =
      "QUERY RESCLASS({$varFour}) SECURITY RESID({$varThree}) RESIDLENGTH({$varSix})"
          + " CONTROL({$varTwo}) USERID({$varFour})LOGMESSAGE({$varFour}) READ({$varSix})"
          + " ALTER({$varFive}) UPDATE({$varFour})";

  private static final String QUERY_SECURITY_VALID_SOME_ONE =
      "QUERY SECURITY RESTYPE({$varTwo}) LOGMESSAGE({$varFour}) ALTER({$varFive})"
          + " RESID({$varThree})";
  private static final String QUERY_SECURITY_VALID_SOME_TWO =
      "QUERY RESID({$varThree}) CONTROL({$varTwo}) SECURITY USERID({$varFour})"
          + " RESIDLENGTH({$varSix}) RESCLASS({$varFour})";
  private static final String QUERY_SECURITY_VALID_SOME_THREE =
      "QUERY ALTER({$varFive}) UPDATE({$varFour}) CONTROL({$varTwo}) RESID({$varThree})"
          + " RESTYPE({$varTwo}) SECURITY";
  ...
  private static final String QUERY_SECURITY_INVALID =
      "QUERY {RESTYPE|error1}(1) {RESCLASS|error1}(1) SECURITY RESIDLENGTH({$varSix})"
          + " RESID({$varThree})";
