 ...
 *
 * <p>Test CICS READprev commands. Documentation link: <a
...
public class TestCICSReadNextReadPrev {
  ...
  private static Stream<Object[]> provideReadCommands() {
    ...
    String[] templates = {
      ...
      "FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) UPDATE TOKEN({$varFive})"
          + " SYSID({$varFour}) RBA LENGTH(LENGTH OF {$varFive})",
    };
    return Arrays.stream(commands)
        .flatMap(
            cmd -> Arrays.stream(templates).map(template -> new Object[] {cmd + " " + template}));
  }

  private static final String READNEXT_CONSISTENT_REPEATABLE_INVALID =
      "READNEXT FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) {CONSISTENT|error1}"
          + " {REPEATABLE|error2}";
  private static final String READNEXT_TOKEN_NO_UPDATE_INVALID =
      "READNEXT FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) {TOKEN|error1}(123)";
  private static final String READPREV_TOKEN_NO_UPDATE_INVALID =
      "READPREV FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) {TOKEN|error1}(123)";
  private static final String READNEXT_RBA_RRN_INVALID =
      "READNEXT FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) {RBA|error1} {RRN|error2}";
  private static final String READPREV_RBA_RRN_INVALID =
      "READPREV FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) {RBA|error1} {RRN|error2}";
  private static final String READNEXT_DOUBLE_UPDATE_INVALID =
      "READNEXT FILE({$varFour}) INTO({$varFive}) RIDFLD({$varFive}) UPDATE {UPDATE|error1}"
          + " TOKEN({$varFive})";
  private static final String READPREV_DOUBLE_UPDATE_INVALID =
  ...
  void testReadnextRepeatableInvalid() {
    Map<String, Diagnostic> expectedDiagnostics =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: UNCOMMITTED or"
                    + " CONSISTENT or REPEATABLE or UPDATE",
    ...
    Map<String, Diagnostic> expectedDiagnostics =
        ImmutableMap.of(
            ...
            new Diagnostic(
                ...
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()));
    CICSTestUtils.errorTest(READNEXT_SYSID_WITHOUT_REQUIRED_OPTION, expectedDiagnostics);
  }
