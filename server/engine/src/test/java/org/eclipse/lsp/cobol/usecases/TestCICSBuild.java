...
public class TestCICSBuild {
  private static final String BUILD_ATTACH_ATTACHID_ALL_VALID =
      "BUILD ATTACH ATTACHID({$varFour}) PROCESS({$varFour}) RESOURCE({$varFour})"
          + " RPROCESS({$varFour}) RRESOURCE({$varFour}) QUEUE({$varFour}) IUTYPE({$varFour})"
          + " DATASTR({$varFour}) RECFM({$varFour})";
