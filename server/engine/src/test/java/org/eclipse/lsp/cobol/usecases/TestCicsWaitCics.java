...
public class TestCicsWaitCics {
  ...
  private static final String WAITCICS_INVALID_TWO =
      "WAITCICS ECBLIST({$varOne}) NUMEVENTS({$varTwo}) {PURGEABILITY|error1}({$varFive})"
          + " NAME({$varThree}) {PURGEABLE|error1}";
  private static final String WAITCICS_INVALID_THREE =
      "WAITCICS {PURGEABILITY|error1}({$varFive}) ECBLIST({$varOne}) NUMEVENTS({$varTwo})"
          + " {NOTPURGEABLE|error1}";
