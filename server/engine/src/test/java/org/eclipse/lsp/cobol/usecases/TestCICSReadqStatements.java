...
/**
 * Test CICS Readq td & Readq ts command. Documentation link: <a
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-readq-td">READQ TD Command</a> <a
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-readq-ts">READQ TS Command</a>
 *
...
public class TestCICSReadqStatements {
  ...
  private static final String READQ_VALID =
      "READQ  QNAME({$varFour}) NOHANDLE INTO({$varFour})\n "
          + "LENGTH({$varFour}) NUMITEMS({$varFour})\n "
          + "NEXT SYSID({$varFour})";
  private static final String READQ_QUEUE_INVALID = "READQ {_QUEUE({$varFour})|errorMissingTd_}";
