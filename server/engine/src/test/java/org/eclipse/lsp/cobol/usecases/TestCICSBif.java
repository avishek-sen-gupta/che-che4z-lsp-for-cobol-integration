...
/**
 * Test CICS BIF commands. Documentation link: <a
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-bif-deedit">BIF DEEDIT Command</a> <a
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-bif-digest">BIF DIGEST Command</a>
 *
 * <p>This class tests all variations of the BIF commands found in the links above.
 */
public class TestCICSBif {
  private static final String BIF_DEEDIT_FIELD_VALID = "BIF DEEDIT FIELD({$varFour})";

  private static final String BIF_DEEDIT_FIELD_LENGTH_VALID =
      "BIF DEEDIT FIELD({$varFour}) LENGTH(100)";

  private static final String BIF_DEEDIT_INVALID = "BIF {DEEDIT | errorMissingField }";
  ...
  private static final String BIF_DIGEST_DIGESTTYPE_INVALID =
      "BIF {_DIGEST RECORD(100) RECORDLEN(100) {HEX|errorDigestType}"
  ...
  void testBifDigestDigesttypeInvalid() {
    Map<String, Diagnostic> expectedDiagnostic =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: HEX or BINARY or"
                    + " BASE64 or DIGESTTYPE",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()),
            "errorDigestType2",
            new Diagnostic(
