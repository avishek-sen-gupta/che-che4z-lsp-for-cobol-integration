...
/**
 * Tests CICS PUT CONTAINER (EXCI) statement. Ref:
 * https://www.ibm.com/docs/en/cics-ts/6.1?topic=interface-exec-cics-link-command-exci
 */
public class TestCicsExciPutContainerStatement {
  public static final String TEXT =
      "       IDENTIFICATION DIVISION.\n"
          ...
          + "       PROCEDURE DIVISION.\n"
          + "           EXEC CICS \n"
          + "           PUT CONTAINER('CONTAINER-2')\n"
          + "           CHANNEL('CHANNEL-1')\n"
          + "           FROM({$CONTAINER-AREA})\n"
          + "           FLENGTH({$CONTAINER-VALUE})\n"
          + "           DATATYPE({$CVDA}) \n"
          + "           FROMCODEPAGE({$CONTAINER-VALUE})\n"
          + "           APPEND\n"
          + "           RETCODE({$RETURN-CODE})\n"
          + "           END-EXEC.\n";
