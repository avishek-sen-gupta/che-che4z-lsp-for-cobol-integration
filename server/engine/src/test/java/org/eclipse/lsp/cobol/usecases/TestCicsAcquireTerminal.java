 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=commands-acquire-terminal">ACQUIRE
 * TERMINAL</a>
 *
...
public class TestCicsAcquireTerminal {
  ...
  private static final String SOME_VALID_THREE =
      "ACQUIRE TERMINAL({$varOne}) USERDATA({$varTwo}) USERDATALEN({$varTwo})";
  ...
  private static final String ALL_VALID =
      "ACQUIRE TERMINAL({$varOne}) QALL RELREQ USERDATA({$varTwo}) USERDATALEN({$varThree})";
  ...
  private static final String DEFAULT_INVALID_TWO =
      "ACQUIRE TERMINAL({$varOne}) {USERDATALEN|error1}({$varThree})";
  private static final String DEFAULT_INVALID_THREE =
      "ACQUIRE TERMINAL({$varOne}) {QALL|error1} {QNOTENAB|error1}";
  private static final String DEFAULT_INVALID_FOUR =
      "ACQUIRE TERMINAL({$varOne}) NOQUEUE {RELREQ|error1}";
  ...
  void testDefaultInvalidThree() {
    CICSTestUtils.errorTest(
        DEFAULT_INVALID_THREE,
        ImmutableMap.of(
            "error1",
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: NOQUEUE or QALL or"
                    + " QNOTENAB or QSESSLIM",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText())),
        "SP");
  }
