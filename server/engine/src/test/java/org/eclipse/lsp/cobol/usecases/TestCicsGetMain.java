 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-getmain">GETMAIN Command</a>
 *
 * <p>This class tests all variations of the GETMAIN command found in the link above.
 */
public class TestCicsGetMain {
  private static final String ALL_VALID_ONE =
      "GETMAIN SET({$varOne}) FLENGTH({$varTwo}) BELOW INITIMG({$varThree}) EXECUTABLE SHARED"
          + " NOSUSPEND USERDATAKEY";
  private static final String ALL_VALID_TWO =
      "GETMAIN LENGTH({$varTwo}) SHARED EXECUTABLE INITIMG({$varTwo}) NOSUSPEND USERDATAKEY"
          + " SET({$varOne})";
  private static final String ALL_VALID_THREE =
      "GETMAIN CICSDATAKEY NOSUSPEND SHARED FLENGTH({$varTwo}) SET({$varOne}) EXECUTABLE"
          + " INITIMG({$varTwo}) BELOW";

  private static final String SOME_VALID_ONE =
      "GETMAIN BELOW FLENGTH({$varThree}) SHARED CICSDATAKEY SET({$varOne})";
  private static final String SOME_VALID_TWO =
      "GETMAIN NOSUSPEND LENGTH({$varThree}) SET({$varOne}) INITIMG({$varTwo}) EXECUTABLE";
  private static final String SOME_VALID_THREE =
      "GETMAIN SET({$varOne}) EXECUTABLE FLENGTH({$varTwo}) USERDATAKEY";
  ...
  private static final String INVALID_ONE =
      "GETMAIN SET({$varTwo}) LENGTH({$varThree}) {BELOW|error1} SHARED";
  private static final String INVALID_TWO =
      "GETMAIN {LENGTH|error1}({$varTwo}) SET({$varOne}) {FLENGTH|error1}({$varTwo}) NOSUSPEND";
  private static final String INVALID_THREE =
      "GETMAIN {USERDATAKEY|error1} FLENGTH({$varTwo}) SET({$varOne}) BELOW {CICSDATAKEY|error1}";
  ...
  void testInvalidThree() {
    Map<String, Diagnostic> expectedDiagnostic =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: USERDATAKEY or"
                    + " CICSDATAKEY",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()));
