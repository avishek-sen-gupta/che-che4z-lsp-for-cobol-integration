...
public class TestCicsMove {
  ...
  private static final String MOVE_ALL_VALID_TWO =
      "MOVE CHANNEL(4) AS(2) TOCHANNEL(5) CONTAINER(1)";
  private static final String MOVE_ALL_VALID_THREE =
      "MOVE FROMPROCESS CONTAINER(2) AS(3) TOACTIVITY(4)";
  private static final String MOVE_ALL_VALID_FOUR =
      "MOVE TOPROCESS AS(2) CONTAINER(1) FROMACTIVITY(4)";
  ...
  private static final String MOVE_INVALID_ONE =
      "MOVE CONTAINER(1) AS(2) {CHANNEL|error1}(1) TOPROCESS";
  private static final String MOVE_INVALID_TWO =
      "MOVE {FROMACTIVITY|error1}(2) AS(3) CONTAINER(1) {FROMPROCESS|error1}";
  private static final String MOVE_INVALID_THREE =
      "MOVE FROMPROCESS CONTAINER(1) AS(2) {TOCHANNEL|error1}(3)";
  private static final String MOVE_INVALID_FOUR =
      "MOVE AS(2) CONTAINER(1) {CHANNEL|error1}(3) TOACTIVITY(4)";
  ...
  void testMoveInvalidTwo() {
    Map<String, Diagnostic> expectedDiagnostics =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: FROMPROCESS or"
                    + " FROMACTIVITY",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()));
