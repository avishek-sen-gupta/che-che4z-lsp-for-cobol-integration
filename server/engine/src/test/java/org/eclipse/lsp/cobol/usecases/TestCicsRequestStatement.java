 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-request-passticket">REQUEST
 * PASSTICKET Command</a>
 *
...
public class TestCicsRequestStatement {
  ...
  // Test Strings
  private static final String ENCRYPTPTKT_VALID_1 =
      "REQUEST ENCRYPTPTKT({$varOne}) FLENGTH({$varOne}) ENCRYPTKEY({$varOne})"
          + " ESMAPPNAME({$varOne}) ESMREASON({$varOne}) ESMRESP({$varOne})";
  private static final String ENCRYPTPTKT_VALID_2 =
      "REQUEST ENCRYPTPTKT({$varOne}) FLENGTH({$varOne}) ENCRYPTKEY({$varOne})"
          + " ESMAPPNAME({$varOne})";

  private static final String PASSTICKET_VALID_1 =
      "REQUEST PASSTICKET({$varOne}) ESMAPPNAME({$varOne}) ESMRESP({$varOne}) ESMREASON({$varOne})";
  private static final String PASSTICKET_VALID_2 =
      "REQUEST PASSTICKET({$varOne}) ESMAPPNAME({$varOne})";

  // Invalid Test Strings
  private static final String ENCRYPTPTKT_INVALID =
      "REQUEST {_ENCRYPTPTKT({$varOne}) ENCRYPTKEY({$varTwo}) ESMAPPNAME({$varThree})|errorOne_}";
  private static final String PASSTICKET_INVALID =
      "REQUEST {_PASSTICKET({$varOne}) ESMREASON({$varOne})|errorOne_}";
  ...
  void testPassticketInvalid() {
    ...
    expectedDiagnostics.put(
        ...
        new Diagnostic(
            new Range(),
            "Missing required option: ESMAPPNAME",
            DiagnosticSeverity.Error,
            ErrorSource.PARSING.getText()));
