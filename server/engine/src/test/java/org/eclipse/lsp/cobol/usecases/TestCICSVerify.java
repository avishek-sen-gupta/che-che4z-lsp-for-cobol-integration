 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-verify-password">VERIFY PASSWORD
 * Command</a>
 *
...
public class TestCICSVerify {
  ...
  private static final String VERIFY_TOKEN_VALID_TWO =
      "VERIFY TOKEN({$varOne}) TOKENLEN({$varTwo}) TOKENTYPE({$varTwo}) DATATYPE({$varThree})"
          + " ENCRYPTKEY({$varThree})";
  ...
  private static final String VERIFY_TOKEN_INVALID_TWO =
      "VERIFY TOKEN({$varOne}) TOKENLEN({$varTwo}) {TOKENTYPE|errorOne}({$varTwo}) {JWT|errorTwo}"
          + " {KERBEROS|errorThree}";

  private static final String VERIFY_TOKEN_INVALID_THREE =
      "VERIFY TOKEN({$varOne}) TOKENLEN({$varTwo}) JWT {DATATYPE|errorOne}({$varTwo})"
          + " {BIT|errorTwo} {BASE64|errorThree}";
  ...
  void testVerifyTokenInvalidOne() {
    CICSTestUtils.errorTest(
        ...
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, none provided: TOKENTYPE or BASICAUTH or JWT or"
                    + " KERBEROS",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText())));
  }
  ...
  void testVerifyTokenInvalidTwo() {
    CICSTestUtils.errorTest(
        ...
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: TOKENTYPE or"
                    + " BASICAUTH or JWT or KERBEROS",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()),
            "errorTwo",
            new Diagnostic(
  ...
  void testVerifyTokenInvalidThree() {
    CICSTestUtils.errorTest(
        ...
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: BIT or DATATYPE or"
                    + " BASE64",
                DiagnosticSeverity.Error,
                ErrorSource.PARSING.getText()),
            "errorTwo",
