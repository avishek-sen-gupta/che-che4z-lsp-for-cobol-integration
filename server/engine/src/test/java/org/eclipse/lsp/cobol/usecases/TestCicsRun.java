 ...
 * href="https://www.ibm.com/docs/en/cics-ts/6.x?topic=summary-run">RUN Command</a>
 *
 * <p>This class tests all variations of the RUN command found in the link above.
 */
public class TestCicsRun {
  private static final String DEFAULT_ALL_VALID_ONE =
      "RUN ACQACTIVITY ASYNCHRONOUS INPUTEVENT({$varThree}) FACILITYTOKN({$varTwo})";
  private static final String DEFAULT_ALL_VALID_TWO =
      "RUN ACQPROCESS INPUTEVENT({$varFour}) SYNCHRONOUS";
  private static final String DEFAULT_ALL_VALID_THREE =
      "RUN SYNCHRONOUS ACTIVITY({$varThree}) INPUTEVENT({$varThree})";
  private static final String DEFAULT_ALL_VALID_FOUR =
      "RUN INPUTEVENT({$varThree}) ASYNCHRONOUS FACILITYTOKN({$varTwo}) ACTIVITY({$varThree})";
  private static final String DEFAULT_ALL_VALID_FIVE =
      "RUN FACILITYTOKN({$varTwo}) ACQACTIVITY INPUTEVENT({$varThree}) ASYNCHRONOUS";

  private static final String DEFAULT_SOME_VALID_ONE =
      "RUN ASYNCHRONOUS ACTIVITY({$varThree}) INPUTEVENT({$varThree})";
  private static final String DEFAULT_SOME_VALID_TWO =
      "RUN ASYNCHRONOUS FACILITYTOKN({$varTwo}) ACQACTIVITY";
  private static final String DEFAULT_SOME_VALID_THREE =
      "RUN ACQPROCESS ASYNCHRONOUS INPUTEVENT({$varThree})";
  ...
  private static final String DEFAULT_INVALID_ONE =
      "RUN SYNCHRONOUS ACTIVITY({$varThree}) {FACILITYTOKN|error1}({$varTwo})";
  private static final String DEFAULT_INVALID_TWO =
      "RUN ASYNCHRONOUS {ACQPROCESS|error1}  {ACQACTIVITY|error1} INPUTEVENT({$varThree})";
  private static final String DEFAULT_INVALID_THREE =
      "RUN ACTIVITY({$varThree}) SYNCHRONOUS INPUTEVENT({$varThree}) {ASYNCHRONOUS|error1}";
  private static final String DEFAULT_INVALID_FOUR = "RUN {ACQACTIVITY|error1}";

  private static final String TRANSID_ALL =
      "RUN TRANSID({$varThree}) CHANNEL({$varTwo}) CHILD({$varOne})";
