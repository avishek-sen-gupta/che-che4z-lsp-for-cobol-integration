...
public class TestCicsDelayStatement {
  ...
  private static final String DELAY_VALID = "DELAY REQID({$varFour})";
  ...
  private static final String DELAY_FOR_MISSING_INVALID = "DELAY {FOR|errorMissingTimeIntervalFor}";
  ...
  void testDelayForUntilInvalid() {
    Map<String, Diagnostic> expectedDiagnostic =
        ImmutableMap.of(
            ...
            new Diagnostic(
                new Range(),
                "Exactly one option required, options are mutually exclusive: INTERVAL, TIME,"
                    + " UNTIL, FOR",
